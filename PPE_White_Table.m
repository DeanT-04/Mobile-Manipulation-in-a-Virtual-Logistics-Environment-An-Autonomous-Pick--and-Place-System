...
if clientID > -1
    ...
    goalLocation = [-0.38, 1.1, pi/2]; % Example target pose
    ...
    executePathPID_WHITE(clientID, sim, path, pioneerHandle, leftMotorHandle, rightMotorHandle);
    ...
    % Swivel robot to +90 degrees for white table
    swivelRobot(clientID, sim, pioneerHandle, leftMotorHandle, rightMotorHandle, 'white');
    pause(1); % Brief pause to observe the swivel before stopping
    % Stop simulation
    % sim.simxStopSimulation(clientID, sim.simx_opmode_blocking);
    % sim.simxFinish(clientID);
