function executePathPID_WHITE(clientID, sim, path, pioneerHandle, leftMotorHandle, rightMotorHandle)
    % Tuned PID Controller Gains (Even Slower)
    Kp_linear = 0.5;  % Lower than before → Robot moves slower
    Ki_linear = 0.005; % Very low to avoid integral wind-up
    Kd_linear = 0.01;  % Smoother transitions, less oscillation

    Kp_angular = 0.3;  % Lower angular gain for gradual turns
    Ki_angular = 0.004; % Reduced integral effect for better stability
    Kd_angular = 0.0005;  % Reduced to avoid sudden jerks
    ...
    while ~goalReached
        ...
        pause(0.05); % **Increased pause for more stable control**
    end
